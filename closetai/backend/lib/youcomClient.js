...
/**
 * News API - Fetch latest fashion news and trends.
 * @param {string} query - Search query
 * @param {Object} options - { count, freshness, offset }
 * @returns {Promise<{ data, rateInfo }>}
 */
async function getNews(query, options = {}) {
  ...
  const resp = await safeGet('/news', params);
  return { data: resp.data, rateInfo: resp.rateInfo };
}

/**
 * Content API - Extract detailed article content for trend analysis.
 * @param {string} url - Article URL to extract content from
 * @param {Object} options - { format } - 'html' or 'markdown'
 * @returns {Promise<{ data, rateInfo }>}
 */
async function extractContent(url, options = {}) {
  ...
  if (!url) {
    const err = new Error('URL is required for content extraction');
  ...
  const resp = await safeGet('/content', params);
  return { data: resp.data, rateInfo: resp.rateInfo };
}

module.exports = { client, safeGet, search, getNews, extractContent };
