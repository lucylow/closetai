...
const { getNews, extractContent } = require('../lib/youcomClient');
 ...
 */
const getFashionNews = asyncHandler(async (req, res) => {
  const query = req.query.q || req.query.query || 'fashion trends 2026';
  const limit = Math.min(parseInt(req.query.limit, 10) || 10, 20);
  const freshness = req.query.freshness || 'week';

  try {
    const { data, rateInfo } = await getNews(query, { count: limit, freshness });
...
const getArticleContent = asyncHandler(async (req, res) => {
  ...
  try {
    const { data, rateInfo } = await extractContent(url, { format: 'markdown' });
    res.json({
      success: true,
      content: data,
      meta: {
        url,
        source: 'You.com Content API',
        rateInfo,
      },
    });
  } catch (error) {
