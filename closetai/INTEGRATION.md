...
| POST | /api/gen/text2img | Yes | Create async text-to-image generation job |
| POST | /api/gen/edit | Yes | Create async image edit job |
| POST | /api/gen/tryon | Yes | Create async virtual try-on job |
| POST | /api/gen/batch | Yes | Create async batch generation job |
| GET | /api/gen/job/:id | Yes | Get job status and result URL |
| GET | /api/gen/jobs | Yes | List user's generation jobs |
| DELETE | /api/gen/job/:id | Yes | Delete a job and its result |
| POST | /api/gen/upload-url | Yes | Get signed URL for direct upload |
| GET | /api/gen/download/:key | Yes | Get signed URL for download |
| POST | /api/gen/prompts | Yes | Save prompt template |
| GET | /api/gen/prompts | Yes | List saved prompts |
...
## Async Generation API (/api/gen)

ClosetAI provides async generative AI job processing via BullMQ workers. All jobs are queued and processed asynchronously.

### Job Types

- **text2img**: Generate images from text prompts
- **edit**: Edit existing images with AI
- **tryon**: Virtual try-on (person + garment)
- **batch**: Batch processing of multiple generations

### Flow

1. Client requests a signed upload URL via `POST /api/gen/upload-url`
2. Client uploads image directly to S3 (bypasses backend)
3. Client creates job via `POST /api/gen/text2img`, `POST /api/gen/edit`, or `POST /api/gen/tryon`
4. Worker picks up job, calls Perfect Corp API, uploads result to S3
5. Client polls `GET /api/gen/job/:id` for status and result URL

### Prompt Templates

Use `POST /api/gen/prompts` to save prompt templates for reuse. Available templates include:
- `hero_product`: Professional e-commerce product shots
- `outfit_composite`: Personalized outfit composites
- `lookbook`: 4-panel lookbook grids
- `social_promo`: Social media assets
- `tryon_variation`: Virtual try-on variations
- `colorway`: Colorway exploration

See `backend/services/promptTemplates.service.js` for all templates.

### Admin Monitoring

- `GET /api/admin/status`: Queue depth, Perfect Corp credits, recent jobs
- `GET /api/admin/billing`: Stripe usage, users near quota
